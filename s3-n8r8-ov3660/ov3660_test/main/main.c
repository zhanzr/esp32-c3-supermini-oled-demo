...
#define OV3660_SCCB_ADDR 0x3C   /* 7-bit slave address (0x78 write / 0x79 read) */

/* OV3660 uses a 16-bit register address map (like OV5640) */
#define REG_PID   0x300A   /* Product ID (MSB): 0x36 for OV3660 */
#define REG_VER   0x300B   /* Product ID (LSB): 0x60 */
...
{
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "  ESP32-S3 N8R8 OV3660 Device ID Test");
    ...
    ESP_LOGI(TAG, "--- Reading OV3660 device ID at addr 0x%02X ---", OV3660_SCCB_ADDR);
    probe_address(bus, OV3660_SCCB_ADDR);
    ESP_LOGI(TAG, "--- Done ---");
}
