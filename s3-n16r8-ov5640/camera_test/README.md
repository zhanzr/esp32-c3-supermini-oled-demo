# OV5640 Camera Frame Test (ESP32-S3 N16R8 OV5640 Board)

Uses the Espressif `esp32-camera` managed component to initialize the OV5640
...
Arduino `camera_pins.h`, which the vendor's CameraWebServer sketch uses) and
identical to the S3-N8R8 OV3660 board.
...
- PSRAM is required for the frame buffer:
  ...
  `CONFIG_SPIRAM_MODE_OCT=y` (N16R8 = octal 8 MB PSRAM), `CONFIG_SPIRAM_SPEED_80M=y`.
- 16 MB flash: `CONFIG_ESPTOOLPY_FLASHSIZE_16MB=y`.
...
- Sensor detected via `SCCB_Read16(0x300A)<<8 | SCCB_Read16(0x300B)` == `0x5640`.
- OV5640 max resolution is 2592x1944 (`FRAMESIZE_5MP`); this test uses QVGA.
- The ov5640 driver only populates `id.PID`; `id.VER`/`id.MIDH`/`id.MIDL`
  stay 0 (the manual SCCB test reads the real values from the registers).
...
I (819) camera: Camera PID=0x5640 VER=0x00 MIDL=0x00 MIDH=0x00
I (819) camera: Detected OV5640 camera
...
I (1169) camtest: Sensor: pid=0x5640 ver=0x0 mid=0x0000 slv=0x3C
...
The vendor package (`D:\board_database\main-esp32-s3-cam1`) is the Freenove
ESP32-S3 CAM kit: its `CameraWebServer` sketch uses `CAMERA_MODEL_ESP32S3_EYE`,
the same GPIO map, and a hardcoded STA connection (`ssid="TP-LINK_D68D"`) with
the standard esp32-camera MJPEG web server (dynamic OV5640/OV3660/OV2640
support in `app_httpd.cpp`).
